...
def compare(args):
    ...
    cn_client = d1_client.cnclient_2_0.CoordinatingNodeClient_2_0(
        cn_base_url, cert_pem_path=args.cert_pem_path, cert_key_path=args.cert_key_path
    )
...
def compare_one(args, node_id, env_dict, cn_client, cn_base_url):
    ...
    cn_node_pyxb = find_node(cn_base_url, env_dict["base_url"])
    ...
    cn_pid_dict, n_cn_objects = get_object_dict(
        ...
        args.api_major,
        client_args_dict,
        list_objects_args_dict={"nodeId": mn_node_id},
    ...
    print_unique(mn_node_id, cn_node_id, mn_pid_dict, cn_pid_dict, n_max_identifiers)
    print_unique(cn_node_id, mn_node_id, cn_pid_dict, mn_pid_dict, n_max_identifiers)
    print(
...
def print_header(cn_node_pyxb, mn_node_pyxb):
    print("{}: CN @ {}".format(cn_node_pyxb.identifier.value(), cn_node_pyxb.baseURL))
    print("{}: MN @ {}".format(mn_node_pyxb.identifier.value(), mn_node_pyxb.baseURL))
...
def print_node_counts(client, env_dict, cn_base_url, cert_pem_path, cert_key_path):
    ...
    for node_pyxb in d1_client.iter.node.NodeListIterator(client):
        mn_client = d1_client.mnclient_1_1.MemberNodeClient_1_1(
            node_pyxb.baseURL, cert_pem_path=cert_pem_path, cert_key_path=cert_key_path
        )
        try:
...
def find_node(cn_base_url, find_str):
    print('{}: Searching NodeList for "{}"...'.format(cn_base_url, find_str))
    for node_pyxb in d1_client.iter.node.NodeListIterator(cn_base_url):
        if (
            find_str.lower() in node_pyxb.identifier.value().lower()
            or find_str.lower() in node_pyxb.baseURL.lower()
        ):
            return node_pyxb
...
def get_object_dict(
    ...
    for i, object_info in enumerate(object_list_iterator):
        ...
        if (
            datetime.datetime.now() - last_status_time
        ).seconds > 1.0 or i == n_objects - 1:
            last_status_time = datetime.datetime.now()
            print(
                "{}: Retrieving ObjectList{}: {:.2f}% ({:,}/{:,})".format(
                    node_id_display_str,
                    " for {}".format(node_id_filter_str) if node_id_filter_str else "",
                    (i + 1) / float(n_objects) * 100.0,
                    i + 1,
                    n_objects,
                )
            )
